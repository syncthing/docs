...
.. warning::
  ...
  Please note, ignored files do NOT imply garbage files such as *Thumbs.db*
  or others generated by the operating system. Ignored files imply files which
  are too important to be shared. If a remote device deletes a directory which
  on your side happens to contain an ignored file, syncthing will stall and
  fail to synchronize because the directory still contains files hence cannot be
  removed, which will result in an out of sync state. If you want to mark files
  as garbage files which should be ignored in the sync process, and at the same
  time can be deleted if they prevent directory removal, check the ```(?d)``
  prefix below.
...
-  A pattern beginning with ``!`` prefix negates the pattern: matching files
...
-  A pattern beginning with ``(?i)`` prefix enables case-insensitive pattern
...
-  A pattern beginning with ``(?d)`` prefix enables removal of these files if
